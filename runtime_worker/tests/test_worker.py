...
class FakeShell:
    async def run_session(self, command):
        yield {"sequence": 1, "event_type": "assistant_message", "payload": {"text": "hi"}}
        yield {"sequence": 2, "event_type": "result", "payload": {"result": "done"}}
...
async def test_worker_claims_executes_and_posts_events() -> None:
    ...
    assert [json.loads(item.content)["events"][0]["sequence"] for item in event_requests] == [1, 2]
