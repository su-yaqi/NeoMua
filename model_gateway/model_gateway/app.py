...
async def resolve_runtime_route(
    runtime_id: uuid.UUID, model_id: str, token: str
) -> dict[str, Any]:
    ...
    if response.status_code != 200:
        raise HTTPException(
            ...
            response.json().get("detail", "Route resolution failed"),
        )
    ...
    async with httpx.AsyncClient(timeout=None) as client:
        async with client.stream("POST", url, headers=headers, json=body) as response:
            if response.status_code >= 400:
                ...
                raise HTTPException(
                    response.status_code, payload.decode(errors="replace")
                )
...
async def messages(
    ...
    try:
        claims = verify_gateway_scope(token, x_runtime_id, model_id)
        runtime_id = uuid.UUID(claims["runtime_id"])
    except (GatewayAuthError, KeyError, ValueError) as exc:
        raise HTTPException(403, str(exc))
    route = await resolve_runtime_route(runtime_id, model_id, token)
