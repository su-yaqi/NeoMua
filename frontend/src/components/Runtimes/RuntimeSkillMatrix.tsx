...
export default function RuntimeSkillMatrix({
  runtimeId,
}: {
  runtimeId: string
}) {
  ...
  if (!query.data?.count)
    return (
      <p className="text-sm text-muted-foreground">尚无 Skill 同步订阅。</p>
    )
  return (
    <div className="space-y-2">
      {query.data.data.map((state) => (
        <button
          ...
          onClick={() =>
            window.location.assign(`/system/skills/${String(state.skill_id)}`)
          }
        >
