...
import { type SkillDraftFile, skillsApi } from "@/api/tenantApi"
  ...
  return (
    <div className="space-y-2 text-sm leading-7">
      {lines.map((line, index) => {
        ...
        if (line.startsWith("```"))
          return (
            <div className="font-mono text-xs text-muted-foreground" key={key}>
              {line}
            </div>
          )
        return (
          <p className={line ? "" : "h-3"} key={key}>
            {line}
          </p>
        )
      })}
...
function Page() {
  ...
  const [section, setSection] = useState<"workspace" | "versions" | "sync">(
    "workspace",
  )
  ...
  useEffect(() => {
    if (!selectedId && files.length) {
      setSelectedId(
        files.find((item) => item.path === "SKILL.md")?.id ?? files[0].id,
      )
    }
  }, [files, selectedId])
  ...
  const dirty =
    Boolean(selected?.is_text) && content !== (selectedContent?.content ?? "")
  useEffect(() => {
  ...
  const selectFile = (fileId: string) => {
    if (dirty && !window.confirm("当前文件有未保存修改，确认放弃并切换？"))
      return
  ...
  const validate = useMutation({
    mutationFn: () =>
      skillsApi.validateDraft(skillId, data?.draft.revision ?? 0),
    onSuccess: async () => {
                  ...
                  <label className="cursor-pointer rounded px-2 py-1 text-xs hover:bg-muted">
                    ...
                    <input
                      ...
                      onChange={async (event) => {
                        ...
                        const version = file
                          ? window.prompt("ZIP 内 SKILL.md 的版本号")
                          : null
                        if (file && version) {
                ...
                >
                  ...
                  {canManage && file.path !== "SKILL.md" && (
                    <span className="hidden gap-1 group-hover:flex">
                      <button type="button" onClick={() => renameFile(file)}>
            ...
            {canManage && (
              <div className="flex flex-wrap items-center justify-between gap-3 border-t bg-muted/20 p-3">
                <div className="text-xs text-muted-foreground">
                  {data.draft.validated_revision === data.draft.revision
                    ? "已验证，可发布"
                    : "内容变更后需要重新验证"}
                </div>
                <div className="flex gap-2">
                    ...
                    !version.deprecated && (
                      <Button
                        ...
                        onClick={async () => {
                          if (
                            window.confirm(
                              `确认将当前版本切换到 v${version.version}？`,
                            )
                          ) {
                            await skillsApi.setCurrentVersion(
                              skillId,
                              version.id,
                            )
                            await refresh()
            ...
            {sync?.data.map((state) => (
              ...
              >
                <span>
                  Runtime{" "}
                  {String(
                    state.runtime_instance_id || state.runtime_profile_id,
                  ).slice(0, 8)}
                </span>
