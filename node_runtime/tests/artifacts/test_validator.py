...
from node_runtime.artifacts.validator import ArtifactValidationError, validate_instruction
...
def test_tampered_manifest_is_rejected(tmp_path) -> None:
    ...
    with pytest.raises(ArtifactValidationError):
        validate_instruction(command, node_id=NODE_ID)
...
def test_cross_node_deployment_is_rejected(tmp_path) -> None:
    ...
    with pytest.raises(ArtifactValidationError, match="node scope"):
        validate_instruction(
            command, node_id="00000000-0000-0000-0000-000000000099"
        )
