...
Admin/Developer 在模板版本配置页选择项目，并为每个非人工节点选择兼容 Runtime；Agent 节点还需选择该 Runtime 上已激活的精确 Agent Release。完整校验后保存为不可变修订。业务用户创建实例时只填写任务名称和“要做什么”，服务端冻结当前配置修订与项目快照；之后模板配置变更不影响已有实例。
