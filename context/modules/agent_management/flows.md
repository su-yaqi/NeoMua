...
1. Admin 通过完整创建流程一次填写 Agent 身份、稳定模型偏好、系统提示词和引擎中立执行策略；服务端原子创建 Agent 与草稿，后续以 CAS revision 保存。Harness 不再是 Agent 可写配置。
...
4. 统一 Resolver 合并 Agent、Skill identity、Plugin、Tool 与 MCP，生成与 engine/provider 无关的 canonical spec；权限只能收紧，冲突携带来源链，缺失能力不 fallback。
...
1. Admin 选择明确 Runtime Instance targets，先调用正式 precheck；不兼容目标返回诊断。在线状态、配置/能力时效、安全上限和模型目录都必须来自服务器事实，不能由前端推断。
