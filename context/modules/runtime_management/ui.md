...
Runtime Instance 和节点详情按 Skill 展示 current desired、实际 applied、generation、状态、最后同步时间与脱敏错误；desired/applied 不一致明确标为 pending/syncing/failed/blocked。Admin 只可对 failed 目标显式重试。任务详情展示 `agent_task_skill_usage` 与任务级模型准备证据，不能把尚未本地复核的快照回显标成可信实际证据。
