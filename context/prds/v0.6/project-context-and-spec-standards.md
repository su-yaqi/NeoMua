...
- [x] Admin/Developer 可从 namespace 成员中建立项目成员集，普通成员不能修改项目配置。
- [x] 项目可绑定多个仓库和多个 Spec 位置，不要求也不推断主仓库。
- [x] 仓库用途使用文字说明；同一仓库可绑定多个项目且系统不擅自改写目录。
- [x] Spec 路径逃逸和仓库外符号链接被拒绝。
- [x] 平台/namespace Spec 标准均使用不可变版本，项目绑定精确版本。
- [x] 标准升级只生成差异和显式迁移动作，不自动覆盖 Git 文件。
- [x] Git 凭证不进入数据库明文字段、API 响应、任务快照或日志。
- [x] 项目只允许归档；归档后不能创建新会话、任务或修改配置。
- [x] v0.6 不从会话或任务自动生成项目记忆。

验收证据：`backend/tests/project_management/test_projects.py` 覆盖管理权限、多个仓库/Spec 位置、路径逃逸、不可变标准版本精确绑定、差异预览、归档只读与持久 Runtime 证明；`backend/tests/runtime/test_workflow_executor.py` 覆盖真实 Git 探测和符号链接逃逸；`frontend/tests/v06-project.spec.ts` 覆盖项目详情展示。
