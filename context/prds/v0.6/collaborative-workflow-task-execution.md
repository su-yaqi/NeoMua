...
- [ ] 界面称项目任务，数据实体为 `workflow_instance`，底层 Agent 执行仍为 `agent_task`，三者不混用。
- [ ] 任务固定模板版本、Package digest、项目上下文和 Runtime 解析，不被新模板版本改变。
- [ ] 项目成员均可参与，负责人不形成排他权限；并发写使用 expected revision 防覆盖。
- [ ] 三种确认模式分别实现人工过程、结果接受和全自动推进，不保留重复的 manual 模式。
- [ ] 上游新修订保留旧结果，并使依赖旧输入的下游逐节点更新；可自动节点自动重跑，人工节点等待确认。
- [ ] Agent 节点在原对话追加明确更新消息并产生新结果版本，不另开替代会话。
- [ ] 节点每次统一调用 `run(context)`，没有强制的首次/更新双接口。
- [ ] 跳过只适用于模板许可节点并记录原因；条件分支不能跳到未定义边。
- [ ] Git 等副作用无法证明状态时停止并请求人工处理，不自动重复、force 或降级。
- [ ] Gate 异常、Runtime 不兼容、Agent/Package digest 不一致均阻断执行。
- [ ] 已完成任务只读；取消、失败、重试和项目归档均保留完整历史。
