...
def upgrade() -> None:
    runtime_type = postgresql.ENUM("platform", "node", name="runtimetype", create_type=False)
    route_mode = postgresql.ENUM("platform_gateway", "direct_anthropic", name="runtimeroutemode", create_type=False)
    task_status = postgresql.ENUM("queued", "dispatched", "running", "cancelling", "succeeded", "failed", "cancelled", "interrupted", "rejected", name="agenttaskstatus", create_type=False)
    event_type = postgresql.ENUM("user_message", "assistant_message", "tool_call", "tool_result", "status", "error", "result", name="agenteventtype", create_type=False)
