...
def test_gateway_can_resolve_direct_runtime_without_exposing_secret_to_browser(
    ...
    created = client.put(
        ...
        json={"route_mode": "direct_anthropic", "model_id": "claude-test",
              "base_url": "https://example.test", "secret_inputs": {"api_key": "secret-value"}},
    ).json()
    token = issue_gateway_token(namespace.id, uuid.UUID(created["id"]), uuid.uuid4(), "claude-test")
    response = client.get(
        f"{settings.API_V1_STR}/internal/runtime/routes/{created['id']}",
        params={"model_id": "claude-test"},
        headers={"X-Runtime-Token": expected_internal_token(), "Authorization": f"Bearer {token}"},
    )
